...
Pulse continuously checks websites and APIs, tracks uptime and latency, detects incidents, and stores monitoring history for a live dashboard.

The backend is split into separate API, scheduler, and worker services so monitoring jobs can be processed asynchronously and reliably.
...
This allows the API, scheduler, and worker to use the same job model while switching queue providers depending on the environment.
...
Statistics are calculated from monitoring history stored in PostgreSQL.
...
Endpoint failures such as HTTP 500 responses, DNS failures, timeouts, and unexpected status codes are stored as failed check results.
...
builds the API, worker, and scheduler Docker images
pushes the images to Amazon ECR
redeploys the three ECS services
Frontend
